...
#[derive(Debug, Clone)]
pub struct Edge {
    pub inner: graph::Edge,
}

impl From<graph::Edge> for Edge {
    fn from(val: graph::Edge) -> Edge {
        Edge { inner: val }
    }
}

impl From<Edge> for graph::Edge {
    fn from(val: Edge) -> graph::Edge {
        val.inner
    }
}

impl<'a> From<&'a Edge> for &'a graph::Edge {
    fn from(val: &'a Edge) -> &'a graph::Edge {
        &val.inner
    }
}
...
impl Edge {
    ...
    pub fn is_selfloop(&self) -> bool {
        self.inner.is_selfloop().into()
    }
}

pub const EDGE_METHODS_NAMES: &[&str] = &["is_selfloop"];

pub const EDGE_TERMS: &[&str] = &["is", "selfloop"];

pub const EDGE_TFIDF_FREQUENCIES: &[&[(&str, f64)]] =
    &[&[("is", 0.1798013), ("selfloop", 0.1798013)]];
...
impl Edge {
    ...
    fn __getattr__(&self, name: String) -> PyResult<()> {
        ...
        let tokens_expanded = tokens
            .iter()
            .map(|token| {
                let mut similarities = EDGE_TERMS
                    .iter()
                    .map(move |term| (*term, jaro_winkler(token, term) as f64))
                    .collect::<Vec<(&str, f64)>>();
        ...
        // where the conribution of each term is weighted by it's similarity
        // with the query tokens
        let mut doc_scores = EDGE_TFIDF_FREQUENCIES
            .par_iter()
            .enumerate()
            // for each document
            .map(|(id, frequencies_doc)| {
                (
                    id,
                    jaro_winkler(&name, EDGE_METHODS_NAMES[id])
                        * frequencies_doc
                            .iter()
                            .map(|(term, weight)| {
                                match tokens_expanded.iter().find(|(token, _)| token == term) {
        ...
        Err(PyAttributeError::new_err(format!(
            ...
            doc_scores
                .iter()
                .map(|(method_id, _)| {
                    format!("* `{}`", EDGE_METHODS_NAMES[*method_id].to_string())
                })
                .take(10)
                .collect::<Vec<String>>()
                .join("\n"),
...
impl Graph {
    ...
    #[automatically_generated_binding]
    #[pyo3(text_signature = "($self, start_node, alpha, iterations, max_walk_length)")]
    /// Estimate the Pagerank vector of the given `start_node` using random walks
    pub fn pagerank_single_node(
        &self,
        start_node: NodeT,
        alpha: f64,
        iterations: usize,
        ...
        self.inner
            .pagerank_single_node(
                start_node.clone(),
                alpha.clone(),
                iterations.clone(),
                max_walk_length.clone(),
            )
            .into()
    }

    #[automatically_generated_binding]
    #[pyo3(text_signature = "($self, node_weights, alpha, iterations, max_walk_length)")]
    /// Estimate the PSEV vector of the given `node_weights` using random walks
    pub fn psev_estimation(
        &self,
        node_weights: HashMap<NodeT, f64>,
        alpha: f64,
        iterations: usize,
        max_walk_length: usize,
    ) -> HashMap<NodeT, f64> {
    ...
    pub fn psev_estimation_nodenames(
        &self,
    ...
    #[pyo3(text_signature = "($self, node_id)")]
    ///
    pub fn get_unchecked_neighbours_node_ids_from_src_node_id(
        &self,
        node_id: NodeT,
    ) -> Py<PyArray1<NodeT>> {
        let gil = pyo3::Python::acquire_gil();
        to_ndarray_1d!(
            gil,
            self.inner
                .get_unchecked_neighbours_node_ids_from_src_node_id(node_id.clone())
                .to_vec(),
            NodeT
        )
    }
    ...
    #[automatically_generated_binding]
    #[pyo3(
        text_signature = "(nodes_pq, edges_pq, nodename_col, edge_src_col, edge_dst_col, nodetype_col, edge_type_col, edge_weight_col, directed, name)"
    )]
...
pub const GRAPH_TFIDF_FREQUENCIES: &[&[(&str, f64)]] = &[
    ...
    &[("estimation", 5.5559187), ("psev", 5.5559187)],
    &[
        ("estimation", 3.6019752),
        ("nodenames", 3.9251726),
        ("psev", 3.6019752),
    ],
    &[("report", 7.7706394)],
    ...
    &[("report", 4.786454), ("textual", 5.5559187)],
    &[("csv", 6.054439), ("from", 1.0252849)],
    &[
        ("connected", 0.98693806),
        ("from", 0.24882871),
        ("id", 0.41163278),
        ("is", 0.8743358),
        ("node", 0.14604768),
        ("unchecked", 0.50361687),
    ],
    &[
        ("connected", 1.3106779),
        ("from", 0.33045062),
    ...
    &[
        ("disconnected", 0.94065243),
        ("from", 0.19357292),
        ("id", 0.32022417),
        ("is", 0.68017775),
        ("node", 0.21163408),
        ("unchecked", 0.39178193),
    ],
    &[
        ("from", 0.24882871),
        ("id", 0.41163278),
    ...
    &[
        ...
        ("node", 0.09074925),
        ("selfloops", 0.5355362),
        ("singleton", 0.45589882),
        ("unchecked", 0.31293106),
        ("with", 0.47461203),
    ],
    &[
        ("from", 0.19357292),
    ...
    &[("remove", 2.9131024), ("tendrils", 5.5559187)],
    &[
        ("dendritic", 3.6019752),
        ("remove", 1.8886027),
        ("trees", 3.6019752),
    ...
    &[
        ("disconnected", 3.230085),
        ("nodes", 1.7390405),
        ("remove", 1.8886027),
    ],
    &[("remove", 2.9131024), ("selfloops", 3.5512767)],
    &[
        ("edges", 2.1842873),
        ("parallel", 3.1031213),
        ("remove", 1.8886027),
    ],
    &[("name", 2.3688278), ("set", 4.786454)],
    &[
        ("all", 1.2377677),
        ("edge", 0.39478353),
        ("inplace", 1.060202),
    ...
    &[
        ("betweenness", 3.1031213),
        ("centrality", 2.270698),
        ("get", 0.23043543),
    ],
    &[
        ("approximated", 0.7382318),
        ("betweenness", 0.9036786),
        ("centrality", 0.6612636),
        ("from", 0.19357292),
        ("get", 0.067106485),
        ("id", 0.32022417),
    ...
    &[
        ("approximated", 0.5896537),
        ("betweenness", 0.7218023),
        ("centrality", 0.52817625),
        ("from", 0.15461402),
        ("get", 0.05360049),
        ("id", 0.25577515),
    ...
    &[
        ("centrality", 2.270698),
        ("eigenvector", 3.6019752),
        ("get", 0.23043543),
    ],
    &[
        ("centrality", 1.561261),
        ("eigenvector", 2.4766057),
        ("get", 0.1584402),
        ("weighted", 1.2870957),
    ],
    &[("get", 0.35543844), ("stars", 6.054439)],
    &[
        ("approximated", 1.742985),
        ("distances", 1.9986851),
        ("get", 0.1584402),
        ("total", 1.742985),
    ],
    ...
    &[
        ("approximated", 2.5349972),
        ("diameter", 3.3890908),
        ("get", 0.23043543),
    ],
    &[
        ("compatible", 2.2209055),
        ("has", 1.0904721),
        ("node", 0.26825),
        ("vocabularies", 2.3302329),
    ],
    &[
        ("compatible", 1.6057988),
        ("has", 0.7884526),
        ("node", 0.1939549),
        ("types", 0.6515289),
        ("vocabularies", 1.6848466),
    ],
    &[
        ("compatible", 1.6057988),
        ("edge", 0.39478353),
    ...
    &[("compatible", 4.982291), ("is", 3.6026523)],
    &[
        ("adjacency", 2.698826),
        ("has", 1.0904721),
        ("matrix", 1.7764648),
    ...
    &[("get", 0.35543844), ("tendrils", 5.5559187)],
    &[
        ("connected", 1.8127376),
        ("generate", 1.8127376),
        ("graph", 1.3607963),
    ...
    &[("components", 4.982291), ("remove", 2.9131024)],
    &[
        ("closure", 3.9251726),
        ("get", 0.23043543),
        ("transitive", 3.9251726),
    ],
    &[
        ("all", 1.7118988),
        ("get", 0.1584402),
    ...
    &[("directed", 2.9665906), ("to", 3.9101431)],
    &[
        ("to", 2.5349972),
        ("triangular", 2.3356469),
        ("upper", 2.7573254),
    ],
    &[
        ("lower", 2.7573254),
    ...
    &[("bidiagonal", 6.054439), ("to", 3.9101431)],
    &[("arrowhead", 6.054439), ("to", 3.9101431)],
    &[("to", 3.9101431), ("transposed", 6.054439)],
    &[("to", 3.9101431), ("undirected", 4.483771)],
    &[("complementary", 6.054439), ("to", 3.9101431)],
    &[
        ("graph", 0.9839073),
        ("multi", 1.9513534),
        ("similarity", 1.9513534),
        ("structural", 1.7906797),
        ("to", 1.2602441),
    ],
    &[
        ("breadth", 0.4350623),
        ...
        ("node", 0.06155835),
        ("parallel", 0.4896234),
        ("predecessors", 0.61933005),
        ("search", 0.4350623),
        ("unchecked", 0.21227194),
    ],
    &[
        ("breadth", 0.4350623),
        ("distances", 0.45866084),
        ("first", 0.4350623),
        ("from", 0.10488003),
        ("get", 0.03635906),
        ("ids", 0.15160805),
    ...
    &[("diameter", 5.227552), ("get", 0.35543844)],
    &[
        ("breadth", 0.80297726),
        ("first", 0.80297726),
        ("from", 0.19357292),
    ...
    &[
        ("between", 0.98695743),
        ("distances", 0.84653217),
        ("get", 0.067106485),
        ("histogram", 0.98695743),
        ("ids", 0.279817),
        ("node", 0.11361581),
        ("type", 0.3066201),
    ],
    ...
    &[
        ("coo", 1.0321888),
        ("get", 0.08626217),
        ("intersection", 1.2686864),
        ("matrix", 0.96718943),
        ("neighbours", 1.2091635),
        ("size", 1.1616354),
    ],
    &[
        ("get", 0.11455827),
        ("graph", 0.9839073),
        ("intersection", 1.6848466),
        ("neighbours", 1.6057988),
        ("size", 1.5426804),
    ],
    &[
        ("ancestors", 1.1220694),
    ...
    &[("connected", 4.066623), ("holdout", 4.483771)],
    &[("holdout", 4.483771), ("random", 3.502471)],
    &[
        ("get", 0.11455827),
        ("holdout", 1.4451251),
        ("indices", 1.9513534),
    ...
    &[("enable", 9.829167)],
    &[("all", 3.8404057), ("disable", 6.054439)],
    &[
        ("default", 2.698826),
        ("graph", 1.3607963),
        ("has", 1.0904721),
    ...
    &[("has", 2.4463217), ("nodes", 2.682408)],
    &[("edges", 3.3691854), ("has", 2.4463217)],
    &[
        ("has", 1.5859827),
        ("nodes", 1.7390405),
        ("trap", 2.6364455),
    ...
    &[("directed", 2.9665906), ("is", 3.6026523)],
    &[
        ("acyclic", 3.6019752),
        ("directed", 1.9232798),
        ("is", 2.3356469),
    ...
    &[
        ("edge", 0.39478353),
        ("has", 0.7884526),
        ("probabilities", 1.9513534),
        ("representing", 1.9513534),
        ("weights", 1.1786183),
    ],
    &[
        ("has", 1.0904721),
    ...
    &[("has", 2.4463217), ("selfloops", 3.5512767)],
    &[
        ("disconnected", 3.230085),
        ("has", 1.5859827),
        ("nodes", 1.7390405),
    ...
    &[("connected", 4.066623), ("is", 3.6026523)],
    &[
        ("has", 1.5859827),
        ("node", 0.39014277),
        ("types", 1.3105587),
    ...
    &[("is", 3.6026523), ("multigraph", 4.786454)],
    &[
        ("has", 1.5859827),
        ("node", 0.39014277),
        ("ontologies", 3.1031213),
    ],
    &[
        ("has", 1.0904721),
        ("node", 0.26825),
    ...
    &[("circles", 6.054439), ("get", 0.35543844)],
    &[
        ("get", 0.1584402),
        ("number", 1.1279974),
        ("of", 1.1279974),
    ...
    &[("get", 0.35543844), ("transitivity", 6.054439)],
    &[
        ("get", 0.08626217),
        ("node", 0.14604768),
        ("number", 0.61413383),
    ...
    &[
        ("clustering", 1.6848466),
        ("coefficient", 1.4451251),
        ("get", 0.11455827),
        ("node", 0.1939549),
        ("per", 1.6848466),
    ],
    &[
        ("clustering", 3.3890908),
        ("coefficient", 2.9068875),
        ("get", 0.23043543),
    ],
    &[
        ("average", 2.698826),
        ("clustering", 2.3302329),
        ("coefficient", 1.9986851),
        ("get", 0.1584402),
    ],
    &[
        ("community", 1.6848466),
        ...
        ("get", 0.11455827),
        ("louvain", 1.9513534),
        ("undirected", 1.4451251),
    ],
    &[
        ("community", 0.98695743),
        ("directed", 0.5600898),
        ("from", 0.19357292),
        ("get", 0.067106485),
        ("memberships", 1.0489528),
        ("modularity", 1.0489528),
        ("node", 0.11361581),
    ],
    &[
        ("community", 0.98695743),
    ...
    &[
        ("get", 0.23043543),
        ("histograms", 3.9251726),
        ("predictions", 3.6019752),
    ],
    &[
        ("filtered", 3.9251726),
        ("get", 0.23043543),
    ...
    &[("from", 1.0252849), ("parquet", 6.054439)],
    &[
        ("approximated", 2.5349972),
        ("cliques", 3.6019752),
        ("get", 0.23043543),
    ...
    &[
        ...
        ("ids", 0.279817),
        ("node", 0.11361581),
        ("structural", 1.0489528),
        ("unchecked", 0.39178193),
    ],
    &[
        ("attachment", 1.3106779),
        ("get", 0.11455827),
        ("minimum", 1.4451251),
        ("preferential", 1.3106779),
        ("unchecked", 0.66881555),
    ],
    &[
        ("attachment", 1.3106779),
    ...
    &[
        ("attachment", 0.98693806),
        ("get", 0.08626217),
        ("minimum", 1.0881765),
        ("preferential", 0.98693806),
        ("unchecked", 0.50361687),
        ("weighted", 0.70075434),
    ],
    &[
    ...
    &[
        ("attachment", 0.7677751),
        ("from", 0.19357292),
        ("get", 0.067106485),
        ("ids", 0.279817),
        ("node", 0.11361581),
        ("preferential", 0.7677751),
        ("unchecked", 0.39178193),
    ],
    &[
        ("attachment", 0.98693806),
    ...
    &[
        ("attachment", 0.61325103),
        ("from", 0.15461402),
        ("get", 0.05360049),
        ("ids", 0.22350042),
        ("node", 0.09074925),
        ("preferential", 0.61325103),
        ("unchecked", 0.31293106),
        ("weighted", 0.43542582),
    ],
    &[
    ...
    &[
        ...
        ("get", 0.05360049),
        ("ids", 0.22350042),
        ("intersection", 0.78832024),
        ("neighbours", 0.75133467),
        ("node", 0.09074925),
        ("size", 0.7218023),
        ("unchecked", 0.31293106),
    ],
    &[
        ("coefficient", 0.84653217),
        ("from", 0.19357292),
        ("get", 0.067106485),
        ("ids", 0.279817),
        ("jaccard", 0.7845368),
    ...
    &[
        ("attachment", 1.8127376),
        ("get", 0.1584402),
        ("preferential", 1.8127376),
        ("scores", 2.2209055),
    ],
    &[
        ("allocation", 1.4451251),
        ("get", 0.11455827),
        ("index", 1.339292),
        ("resource", 1.4451251),
        ("scores", 1.6057988),
    ...
    &[("density", 6.054439), ("get", 0.35543844)],
    &[
        ("get", 0.1584402),
        ("nodes", 1.19571),
        ("rate", 2.0609372),
    ...
    &[
        ("destination", 1.9442365),
        ("get", 0.1584402),
        ("ids", 0.66065544),
        ("node", 0.26825),
    ],
    &[
        ("destination", 1.4057566),
        ("directed", 0.9561359),
        ("get", 0.11455827),
        ("ids", 0.47767887),
        ("node", 0.1939549),
    ...
    &[
        ("arborescence", 2.4766057),
        ("kruskal", 2.4766057),
        ("random", 1.561261),
        ("spanning", 2.2209055),
    ],
    &[
        ("arborescence", 3.6019752),
        ("kruskal", 3.6019752),
        ("spanning", 3.230085),
    ],
    &[
    ...
    &[("add", 4.483771), ("selfloops", 3.5512767)],
    &[
        ("id", 1.0996104),
        ("node", 0.39014277),
        ("validate", 2.8276975),
    ...
    &[
        ("be", 3.1031213),
        ("multigraph", 3.1031213),
        ("must", 2.4897857),
    ],
    &[
        ("be", 2.1336093),
        ("multigraph", 2.1336093),
        ("must", 1.7118988),
        ("not", 2.0609372),
    ],
    &[
    ...
    &[("chains", 6.054439), ("get", 0.35543844)],
];
