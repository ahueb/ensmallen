            ...
            frontiers[..current_depth]
                ...
                .for_each(|(depth, frontier)| {
                    frontier.par_iter().copied().for_each(|src| {
                        ...
                        unsafe {
                            (&mut (&mut *shared_visited_status.get()))[src as usize] = UNVISITED;
                        }
                        let offset = unsafe {
                            self.edges
                                .get_unchecked_minmax_edge_ids_from_source_node_id(src)
                            ...
                            * if current_depth == depth + 1 {
                                ...
                                *number_of_successors as f32
                                ...
                                successors[offset as usize
                                    ...
                                    .iter()
                                    .map(|&dst| {
                                        1.0 + unsafe { (&mut (&mut *shared_dependencies.get()))[dst as usize] }
                                    })
                                    .sum::<f32>()
                            };
                        *number_of_successors = 0;
                        ...
                        unsafe { (&mut (&mut *shared_dependencies.get()))[src as usize] = dependency };
                        ...
                        unsafe { (&mut (&mut *shared_centralities.get()))[src as usize] += dependency };
                    });
                });
        });
                        ...
                        let dependency: f32 = path_counts
                            * if current_depth == depth + 1 {
                                ...
                                successors[offset as usize
                                    ...
                                    .map(|&dst| {
                                        (1.0 + unsafe {
                                            (&mut (&mut *shared_dependencies.get()))[dst as usize]
                                        }) / shortest_path_counts[dst as usize]
                                            .load(Ordering::Relaxed)
                                            as f32
                                    })
