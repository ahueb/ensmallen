            ...
            frontier.par_iter().for_each(|node_id| {
                self.iter_unchecked_neighbour_node_ids_from_source_node_id(*node_id)
                    .for_each(|neighbour_node_id| {
                        if (&mut *thread_shared_predecessors.value.get())[neighbour_node_id as usize]
                            == NODE_NOT_PRESENT
                        {
                            // Set it's distance
    ...
    >(
        ...
        while !frontier.is_empty() {
            ...
            frontier.par_iter().for_each(|node_id| {
                self.iter_unchecked_neighbour_node_ids_from_source_node_id(*node_id)
                    .for_each(|neighbour_node_id| {
                        if (&mut *thread_shared_distances.value.get())[neighbour_node_id as usize]
                            == node_not_present
                        {
                            // Set it's distance
    ...
    pub unsafe fn get_unchecked_eccentricity_and_most_distant_node_id_from_node_id(
        ...
        while !frontier.is_empty() {
            ...
            frontier.par_iter().for_each(|node_id| {
                self.iter_unchecked_neighbour_node_ids_from_source_node_id(*node_id)
                    .for_each(|neighbour_node_id| {
                        if !(&mut *thread_shared_visited.value.get())[neighbour_node_id as usize] {
                            // Set it's distance
